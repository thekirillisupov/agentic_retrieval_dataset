...
MECHANICS: dict[str, dict[str, str]] = {
    ...
    "verbatim_lookup": {
        "plan": "Вход в задачу — точный идентификатор (номер, код, обозначение): "
                "лексический поиск находит его за один запрос, плотностный — нет. "
                "Учит выбирать инструмент, а не всегда бить в dense.",
        "compose": "Вопрос ОБЯЗАН содержать точный идентификатор (номер закупки, "
                   "код, обозначение), скопированный дословно из цитаты факта. "
                   "Ответ при этом требует второго фрагмента, который по одному "
                   "идентификатору не находится.",
    },
}
...
SUBMECHANICS: dict[str, list[str]] = {
    ...
    "comparison": [
        ...
        "план против факта",
        # the light tier's harder cousins: naming both sides makes comparison
        # 2x single-hop, so a share of the quota pins one side behind a
        # description the agent must resolve first
        "одна сторона названа, вторая задана только описанием",
        "обе стороны заданы описанием без имён собственных",
    ],
    "temporal_resolution": [
    ...
    "verbatim_lookup": [
        "точный номер документа или закупки",
        "код классификатора или статьи",
        "кодовое обозначение изделия или версии",
        "идентификатор + свойство из связанного фрагмента",
        "точный номер + период или место",
    ],
}
